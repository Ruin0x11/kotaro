...
function god_blessing:new(field)
   return setmetatable({field = field}, {__index = god_blessing})
end
function god_blessing:applies_to(node)
   if node:type() ~= "suffixed_expression" then
      return false
   end

   if node:name():raw_value() ~= "make_skill_blessing" then
      return false
   end

   return true
end
function god_blessing:execute(node)
   local it = node:arguments():at(1):evaluate()

   local found
   for _, v in ipairs(skill) do
      if v.elona_id == it then
         found = "elona." .. v._id
         break
      end
   end
   ...
   local expr = Codegen.gen_expression("\"" .. found .. "\"")

   node:arguments():at(1):replace_with(expr)
   node:arguments():changed()
end

return {
   execute = function(self, ast, params, opts)
      return ast:rewrite(god_blessing:new())
   end
}
