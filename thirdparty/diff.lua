...
local IN   = "in"; local OUT  = "out"; local SAME = "same"  -- token statuses
...
-----------------------------------------------------------------------------
function diff.diff(old, new, separator)
   ...
   for i = #rev_diff, 1, -1 do
      table.insert(the_diff, rev_diff[i])
   end
   the_diff.to_html = format_as_html
   return the_diff
end
